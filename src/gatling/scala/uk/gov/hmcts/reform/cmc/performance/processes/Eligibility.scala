...
object Eligibility {
  ...
  def run(implicit postHeaders: Map[String, String]): ChainBuilder = {
    ...
    exec(http("Eligibility start")
      .get(eligibilityPath))
      .pause(2)
      .exec(http("Eligibility - claim on behalf GET")
        .get(s"$eligibilityPath/claim-on-behalf")
        .check(CsrfCheck.save))
      .exec(http("Eligibility - claim on behalf POST")
        .post(s"$eligibilityPath/claim-on-behalf")
        .formParam(csrfParameter, csrfTemplate)
        .formParam("claimOnBehalf", "no")
      ...
      .exec(http("Eligibility - claim value POST")
        ...
        .check(CurrentPageCheck.save)
        .check(CsrfCheck.save))
      .exec(http("Eligibility - single claimant POST")
        .post(currentPageTemplate)
        .formParam(csrfParameter, csrfTemplate)
        .formParam("singleClaimant", "yes")
        .check(CurrentPageCheck.save)
        .check(CsrfCheck.save))
